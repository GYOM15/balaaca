...
4. **A window may span midnight, and the schema must let it.** The constraint
   ...
   resolved in `LocalWindows.window`, which advances `endDate` by one local day
...
14. **These edge cases have named tests, and a slot calculator arriving without
    ...
    directions, spring gap and autumn fold; and — against real PostgreSQL —
    a property test asserting that **every slot the calculator proposes inserts
