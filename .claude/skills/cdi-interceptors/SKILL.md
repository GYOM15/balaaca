...
4. **Order interceptors explicitly with `@Priority`, and write the number
   ...
   `PLATFORM_BEFORE + 200` is Quarkus's own transactional interceptor —
   it is not ours to move. Audit sits at `+210`, inside the transaction,
...
6. **An interceptor binds `TenantContext`; it cannot bind the database
   ...
   also covers every transaction opened without `@TenantBound` —
   `notification-worker` jobs, scheduled tasks, admin paths. Matching
...
8. **No inline logging in intercepted classes.** Domain, application and
   ...
   as `appointment.booked` and `appointment.book.slot_unavailable` —
   always through the masking helper. `provider_id` and `correlation_id`
...
10. **Interceptors carry no persistent state.** Any store they consult
    ...
    swappable. No `static` map, no field mutated across invocations —
    interceptor instances are shared.
