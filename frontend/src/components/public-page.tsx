 ...
 *
 * <p>Duplicating the markup for the preview was the obvious alternative and it
 * is the one that rots: two pages claiming to be one, drifting apart on the
 * first change either side. A field added here appears in the preview because
 * it cannot not.
 *
 * <p>The chrome is NOT here. The public route wraps this in the site header and
 * the tab bar; the preview wraps it in the dashboard's own. Everything between
 * them is the page.
 *
 * @param preview draws the page without the things a reader cannot do yet. The
 *                booking button is the whole of it: on an unpublished page it
 *                would lead to a route that answers 404, and a preview whose
 *                only interactive element is broken teaches the wrong lesson
 *                about the page it is previewing
 */
export function PublicPageBody({
  ...
  // A preview of an unpublished page must not offer a route that answers 404 -
  // which is the defect this whole component exists to undo, reappearing one
  // level down. It leads to the publish switch instead, which is what the
  // reader actually needs from there.
  const bookHref = preview ? "/dashboard/profile" : `/p/${slug}/reserver`;
  ...
  return (
    <main id="contenu" className={preview ? undefined : "has-tabbar"}>
...
function Review({ review }: { review: ReviewType }) {
  return (
    <article className="review">
      ...
      {/* Under the review and indented, never level with it. An answer is a
          reply to something; laid out side by side it reads as a second
          opinion of equal standing, which is not what it is. */}
      {review.reply ? (
        <div className="review__reply">
          <p className="t-overline">Réponse du professionnel</p>
...
export function tradeLabel(categories: CategoryList, slug: string | undefined): string | undefined {
